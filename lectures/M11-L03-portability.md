...
activity_question: "Take a sequential `gensym` that uses a `ref` counter. Now imagine handing it to two domains in parallel. What goes wrong at runtime, and what would it take for the *type checker* to refuse the program before any thread is created?"
...
The previous lecture (M11-L02) gave us **locality**: a type-level
discipline that makes stack allocation safe. The compiler tracks
which values must not escape their region; allocation on the
stack is then sound, because the compiler refuses programs that
would let a stack pointer outlive its frame.

This lecture moves to the next ingredient of OxCaml's safety
story: making *concurrency* data-race-free at the type level.
...
(M11-L06). The portability axis asks "can this value safely cross
a domain boundary?" If a closure captures thread-local mutable
state, the answer is no, and the type checker rejects any attempt
to send it to another domain. The race never reaches runtime,
because the program never compiles.
...
- Stack-escape: M11-L02 closed by locality.
...
- M10: GC + types rule out memory bugs by construction.
- M11-L02: **locality**. Stack allocation is safe at the type
  level.
- This lecture (M11-L03): **portability**. The next ingredient,
  data-race freedom across domains, also at the type level.

:::

## The motivating example: a parallel gensym race

The cleanest motivation comes from a problem you have already met
in CS3100-flavoured systems code: generating unique symbols. A
sequential `gensym` is a one-liner: hold a counter in a `ref`,
increment on every call, format the result.
...
in a hot loop. The four ingredients of a data race (recall from
your concurrency reading or the CS6868 handout) are present:
...
```ocaml skip
(* In OCaml 5 today, this compiles and races at runtime. *)
...
ingredient 3 with **contention** (M11-L06).
...
```ocaml skip
val Domain.Safe.spawn :
  (unit -> 'a) @ portable -> 'a Domain.t
```

Read the signature carefully. The argument is a thunk
`unit -> 'a`, but with a mode annotation: `@ portable`. The thunk
must be portable; only portable closures can be handed to another
domain.
...
```ocaml skip
(* OxCaml refuses this at compile time. *)
let _ = Domain.Safe.spawn (fun () -> gensym "x")
(* Error: The value gensym is nonportable but is expected to be
   portable because it is used inside the function at ...
...
- The thunk must be `portable`.
- The vanilla `Domain.spawn` lacks the annotation, which is why
...
The mechanics of the rule are worth understanding. A closure
captures the bindings in its enclosing scope. Each captured
binding has a mode on every axis. The closure's mode on the
portability axis is determined by what it captures:

- Capture nothing mutable: the closure is **portable**.
- Capture an `int`, a `string`, a `float`, any immutable value:
  the closure is **portable**.
- Capture a `ref`, a mutable record, a `Hashtbl.t`, any value
  carrying thread-local mutable state: the closure is
  **nonportable**.

`gensym`'s closure captures `count : int ref`. `int ref` is the
canonical thread-local mutable cell. So `gensym`'s closure is
nonportable, and the spawn fails.

The intuition: if the closure ran on another domain, the original
domain might still call `gensym` directly. Both domains would
write to `count`. Ingredient 3 of a data race (two writers) is
exactly what the compiler is refusing to permit.
...
- Capture an immutable value: closure stays portable.
- Capture a `ref` (or other thread-local mutable state):
  closure becomes nonportable.
- The compiler can name the offending capture in the error
  message.

`gensym` captures `int ref`. Therefore nonportable. Therefore
cannot be spawned.
...
But OxCaml's compiler is not satisfied yet:
...
mode-crosses the **contention** axis (the topic of M11-L06): it is
...
- `Atomic.t` mode-crosses *contention* (M11-L06).
...
OxCaml ships a dedicated `Portable.Atomic` whose `t` mode-crosses
*both* contention and portability. A closure that captures a
`Portable.Atomic.t` instead of a stdlib `Atomic.t` is portable; it
can be handed to `Domain.Safe.spawn` without complaint.
...
let d1 = Domain.Safe.spawn (fun () -> gensym "x")
let s2 = gensym "y"
let s1 = Domain.join d1
let () = Printf.printf "%s %s\n" s1 s2
```

Two domains, a shared atomic counter, no race; the compiler
verified all of that before we ran it. The `@ portable`
annotation on `res` forces the proof: each capture must
mode-cross portability, and `Portable.Atomic.t` does. `count` is
created once when `gensym` is bound, shared by every invocation
across every domain, and the closure handed to
`Domain.Safe.spawn` captures only the portable `gensym` and
nothing else.
...
- Swap stdlib `Atomic` for `Portable.Atomic`.
- Annotate the closure `@ portable`.
- The compiler checks every capture mode-crosses portability.
...
A function `f : int -> int` is defined as follows:
...
Why is `f` nonportable?

- [ ] It returns an `int`, which is not a portable type.
- [x] It captures `log : Buffer.t`, which is thread-local mutable
      state.
...
Mutable state is by default `nonportable`, and a closure that
captures `nonportable` data is itself `nonportable`. The return
type, the helper calls, and the top-level position are not the
issue: it is the capture of the mutable buffer. The compiler
will point at the buffer in its diagnostic.
:::
...
- Portability is about **captures**: a closure becomes
  nonportable when it captures thread-local mutable state.
...
**Pitfall 2: "If a closure is pure, it must be portable."** Pure
closures are portable. The compiler proves it from the captures,
not from any analysis of the body. If your closure captures
mutable state but never writes through it, the compiler still
considers it nonportable.
...
is accessed (one domain or many). M11-L06 develops the contention
...
The next axis (contention, M11-L06) attacks race ingredient 3.

:::

## What's next

The next lecture (M11-L04) is **uniqueness**: a separate axis on
the resource-management side of the mode system. After uniqueness
comes **linearity** (M11-L05), then **contention** (M11-L06), the
other half of the data-race-freedom story. M11-L07 is the
tutorial that combines the resource axes (uniqueness, linearity,
locality) with portability and contention in a single API.
...
- Lecture 6: **contention**. Cross-domain access. Atomic mode
  crossing.
- Lecture 7: tutorial. A resource-management API that ships
...
M11-L02's locality is original to this course. See
