...
concepts: [contention, uncontended, contended, shared, Atomic, mode crossing, spinlock, parallel counter]
keywords: [OCaml, OxCaml, contention, uncontended, contended, Atomic, mode crossing, parallel counter, spinlock]
activity_question: "A parallel-sum program accumulates into a single `int ref` from two domains. The total comes out wrong. Rewrite it so OxCaml accepts the program; what mode annotations do you need?"
...
In M11-L03 we met **portability**: the axis that decides whether
a value can cross a domain boundary at all. The rule there was
clear. A closure capturing a `ref` cannot be spawned, because the
closure is `nonportable`; replacing the `ref` with a
`Portable.Atomic.t` makes the closure portable, and the spawn
goes through.
...
- M11-L03's portability: can the value *cross* a domain
  boundary?
...
- M11-L01: modes overview, five axes.
- M11-L02: locality. Scope escape.
- M11-L03: portability. Cross-domain crossing.
- M11-L04: uniqueness. Past aliasing.
- M11-L05: linearity. Future use.
- This lecture (M11-L06): **contention**. Cross-domain access.
...
- **Portability** (M11-L03): can this value *get to* another
  domain at all?
- **Contention** (M11-L06): once it gets there, how can it be
  *accessed*?
...
- **Portability** (M11-L03) attacks ingredient 2: by rejecting
...
| 2 | Shared location | **Portability** (M11-L03) |
...
(* Error: This value is contended but is expected to be shared or
   uncontended because its mutable field mood is being read. *)
```
...
```ocaml skip
(* Compiles in OCaml 5 today; races at runtime. *)
let count = ref 0
...
(* Expected 2_000_000; actually some value below it. *)
```
...
The fix is `Atomic.t`, which mode-crosses contention:
...
let () =
  let d1 = Domain.Safe.spawn (fun () -> bump_loop 1_000_000) in
  let d2 = Domain.Safe.spawn (fun () -> bump_loop 1_000_000) in
  Domain.join d1;
  Domain.join d2;
  Printf.printf "count = %d\n" (Atomic.get count)
```

`Portable.Atomic.t` mode-crosses both portability (so the
closure can be spawned) and contention (so the increment is
safe). The final count is exactly 2,000,000; the compiler proved
it before the runtime ran.

:::slide

## From mutex convention to typed pattern

- CS6868 Lec05: locking was a runtime convention. `acquire`,
  work, `release`. Forgetting any step: silent race.
- TSan was the safety net, at runtime only.
- OxCaml contention: lock structure encodes the protocol; the
  type checker rejects forgotten / mis-ordered locking.
- `with_lock t (fun data -> ...)` is the typed pattern.
...
For state more complex than an atomic counter (a hash table, a
linked list, a database connection pool), atomics are not
enough. The standard runtime answer is a mutex; the OxCaml
answer is a *typed* version of the same pattern, where the lock
hands out an `uncontended` view of the protected data only
inside the critical section.

A minimal sketch (the full pattern lives in OxCaml's `Capsule`
library, which the CS6868 handout covers in Part 5):

```ocaml skip
(* Toy spinlock, schematic. The real one uses Capsule.t. *)
module Spinlock = struct
  type 'a t
  val make : 'a -> 'a t @ portable
  val with_lock :
    'a t @ portable -> ('a @ uncontended -> 'b) @ portable -> 'b
end
```

Read the signature. The `Spinlock.t` itself is `portable`: you
can ship it to any domain. The `with_lock` operation hands the
callback an `@ uncontended` reference to the protected data,
*inside* the lock. The callback is itself portable, so it can run
on any domain, but inside `with_lock` it has exclusive access.

The contrast with the manual locking from CS6868 Lecture 05 is
sharp. There, the lock was a runtime convention: you remembered
to call `acquire` and `release`, and the compiler did not check.
Here, the lock structure is *the* way to get from `contended` to
`uncontended`: the type system insists you go through `with_lock`
to get write access, so forgetting the lock is impossible.
...
module Spinlock = struct
  ...
  val with_lock :
    'a t @ portable ->
    ('a @ uncontended -> 'b) @ portable -> 'b
end
```

- `Spinlock.t` itself is portable.
- `with_lock` hands the callback `@ uncontended` access *inside*
  the lock.
- Forgetting to lock is impossible: there is no other way to
  get `uncontended` access.

Manual locking (CS6868 Lec05) becomes a typed pattern.
...
same point made in M11-L03 from the portability side.
:::
...
is a separate question (portability, from M11-L03). For both
guarantees together, the type is `Portable.Atomic.t`.
...
- Race ingredient 3 (mutable read or write) becomes a
  compile-time check.
- Manual mutex discipline becomes a typed pattern (`with_lock`).
- `Atomic.t` is the typed shorthand for "safe to share without
  the mutex."
- Together with portability (M11-L03), all four race
...
The next (and final) lecture of M11 is the tutorial (M11-L07).
It puts the four resource axes (locality, portability,
uniqueness, linearity, contention) together in a single API: a
resource-management module that is safe to use within a scope,
safe to share across domains, and safe to free exactly once.
After M11 comes M12, where MirageOS uses these disciplines for
systems-level safety (unikernels written in OCaml end to end).

:::slide

## What's next

- Lecture 7: tutorial. A resource-management API that combines
  the resource axes (locality, uniqueness, linearity) with the
  concurrency axes (portability, contention).
- Module 12: MirageOS unikernels. The same disciplines, applied
  to whole operating systems written in OCaml.
...
typed-spinlock sketch, and the
`Atomic.t`-versus-`Portable.Atomic.t` story are adapted from the
CS6868 OxCaml handout, Part 2 (the instructor's own teaching
material, freely reusable). The framing of contention as the
"second half" of OxCaml's race-freedom story, and the
ingredient-by-ingredient table in this lecture, are original to
this course. See
